...
This is almost there, but the column names don't reflect the fact that these
vars now represent _averages_ of means/standard deviations.

While we're cleaning up these varnames, let's get rid of the parenthesis
and the (I assume mislabeled, since it doesn't match the provided codebook 
features_info.txt) `fBodyBody` in some of the vars.
...
Since we need to commit/submit this one, it's stored in the project root
